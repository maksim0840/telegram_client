...
std::string Message::get_text_with_options() {
    ...
    std::snprintf(text_with_options, MAX_TELEGRAM_MESSAGE_LEN, "[/%i%i%i%i%i%i%i-%i-%i-%i-%i] %s", \
        rsa_init, rsa_form, rsa_use, aes_init, aes_form, aes_use, dh_fastmode, rsa_key_n, aes_key_n, last_peer_n, rsa_key_len, text.c_str());
